...
module UserService
  class SyncWithLoops
    ...
    def run
      ...
      # FUIME-DIVERGENCE: upstream synced teenagers to a Hack Club Airtable base
      # (including legal name, date of birth, and home address) and only adults to
      # Loops. Fuime has no Airtable, so every user syncs to Loops.
      body = {
        email: @user.email,
      ...
      body[:userGroup] = @user.teenager ? "Teen" : "Adult"
      body[:subscribed] = true if @contact_details.nil?
      body[:source] = "Fuime" if @contact_details.nil?
