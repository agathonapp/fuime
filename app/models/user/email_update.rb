  ...
  class EmailUpdate < ApplicationRecord
    ...
    def non_hcb_email
      if GSuiteAccount.where(address: replacement).any? || GSuiteAlias.where(address: replacement).any?
        errors.add(:email, "must not be provided through an Fuime account's Google Workspace")
      end
    end

    def non_existing_email
      if User.where(email: replacement).any?
        errors.add(:email, "is currently in use on Fuime, please use another address")
      end
    end
