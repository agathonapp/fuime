    ...
    class Stats < Metric
      ...
      def calculate
...

        {
          ...
          mobile_installs: ::ApiToken.joins("INNER JOIN oauth_applications ON api_tokens.application_id = oauth_applications.id").where(oauth_applications: { name: "Fuime Mobile" }).distinct.count(:user_id)
        }
      end
