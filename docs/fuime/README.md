...
| **ONBOARDING_WIZARD_SPEC.md** | The design for the family setup wizard (both entry orders), with the parallel review's signup findings appended. | **History now.** The wizard is built; `UPSTREAM_DIVERGENCE.md` "2026-09-12 — The family setup wizard" is what shipped, and it differs from this document wherever the constraint audit found a problem (the consent service, the parent-door refusal, the revoked pair, the payout copy). | Sep 2026. Superseded in the four places the divergence entry names. |
...
| **PLATFORM_REVIEW_2026_09.md** | The 2026-09-12 deep review: 174 findings from 17 reviewers across money, security, legal copy and UI, checked against a 98-page screenshot walk of every persona. Carries a status header mapping each shipped row to the commit that closed it. | **The current punch list.** Start at its §2 (P0) and §3 (P1); the status header says what not to redo. | Sep 2026. Current. |
| **ADMIN_OPS_QUEUES.md** | Spec for the admin queues Fuime's own money model needs (failed payouts, connected-account status, stuck guardianships, past-due subscriptions). | **§3 and §4 built** (guardian invites + subscriptions). §1–§2 still unbuilt; they wait on G10. | ⚠️ §2 (connected accounts) is **obsolete under MoR** and §1 needs the smaller rewrite in `ADMIN_CONSOLE_MAP.md` §7. §3/§4 rows are current. |
| **ADMIN_CONSOLE_MAP.md** | Every admin page classified — live / orphaned / permanently empty / broken — with the model that backs it and the grep proving what writes it. Plus the HCB-vocabulary list, the exact `config/routes.rb` removal list with a `_path`-helper safety audit, and a reassessment of the two unbuilt ops queues against merchant-of-record. | **What to delete from the admin console, and what not to.** Start at its §1 (the one live bug) and §4 (the route removals). | Sep 2026. Current. Read-only audit; line numbers drift because three agents were editing `routes.rb` alongside it — match on the quoted text. |
