...
| **ONBOARDING_WIZARD_SPEC.md** | The synthesized design for the new multi-screen teen + parent onboarding wizard (both entry orders), with the parallel review's signup findings appended. | **Build from this** once its constraint audit has run (it has not). Verify every route/method by grep. | Sep 2026. Design only; nothing built. |
