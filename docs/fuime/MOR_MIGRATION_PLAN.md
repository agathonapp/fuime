...
| `app/models/legal_entity/payout_method/stripe_connect.rb` | **ADD** | **critical** — the only working rail (§0.3) |
...
**4.2 No working rail.** All four `PayoutMethod` subclasses run on Column/Increase/Wise. A new
`LegalEntity::PayoutMethod::StripeConnect` — Stripe `Transfer` to an Express account owned by
the operator's guardian, or a Stripe `Payout` to a verified external account — is the one
genuinely new piece of money-movement code in this plan. Note the irony worth stating plainly:
**you still need Connect**, just Express-for-payouts rather than Standard-for-collection, and
the guardian still has to complete Stripe KYC to receive money (L3 is unavoidable either way).
