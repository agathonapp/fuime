...
| Onboarding button text | "Start using HCB" → "Start using Fuime" → "Let's go" (2026-09-11, onboarding Phase A) | `app/views/users/edit.html.erb` |
...
and the parent read did not describe the product that runs. Production is
merchant-of-record: a teen can set up and, once Fuime's human vetting approves
the venture, sell before a parent has accepted; the guardian gates payouts,
which run as a weekly admin-approved `Fuime::PayoutBatch`. The site, the signup
page, the guardian invite, the accept page, the agreement and the FAQ all still
said some mix of "test mode / no real money", "parked until a guardian",
"activate their account", "you approve every payout" and "can no longer operate
a business". Phase A makes every one of those sentences true, removes the
fields signup did not need, sends the two emails the copy already promised, and
gives a parent a way out of an expired link. No ledger, model schema or money
path changed. Every sentence that differs between MoR and Connect branches on
`::Fuime::Features.merchant_of_record?`; the versioned agreement and the static
site cannot branch and are written to be true under both.
...
| Guardian invite email rewritten (subject "{Teen} started a business — they need a parent to sign off"; venture name from latest Event/Application; MoR: "can keep going without waiting on you. They can't be paid until you sign"; no-SSN/ID/payment line MoR-only); reminders share the body; `#accepted` to the teen no longer says "fully activated" under MoR | The reason to act was only in the day-3/6 reminders; the first email promised notifications and a tax summary that do not exist | `app/mailers/guardianship_mailer.rb`, `app/views/guardianship_mailer/{invite,invite_reminder,accepted}.{html,text}.erb`, `spec/mailers/guardianship_mailer_spec.rb` |
| `/guardian/new`: no "Almost there!" / "fully activated"; MoR: "You can keep setting up. Your parent or guardian signs before you get paid."; create flash split the same way | False under MoR | `app/views/guardianships/new.html.erb`, `app/controllers/guardianships_controller.rb` |
| Accept page: H1 "{Teen} needs a parent to sign off[ on {Venture}]", three true bullets (MoR: "Nothing is paid out until you've accepted and set up where the money goes"), button "I agree", note adds "This is not an identity check"; checkbox label and agreement partial byte-for-byte unchanged | "Agree & activate their account" and "you approve every payout" are false under MoR (weekly batch, admin-approved) | `app/views/guardianships/show.html.erb`, `spec/controllers/guardianships_render_spec.rb`, `spec/requests/family_signup_flow_spec.rb` |
| Footer disclosure no longer hidden on `/guardian/:token`, `/guardian/new`, expired page | 12 CFR 328.102(b)(3)(ii): omission is the violation; the accept page is the reader it exists for | `app/controllers/guardianships_controller.rb`, `spec/requests/fuime/status_disclosure_spec.rb` |
| Expired link → `guardianships/expired` (410) with POST `/guardian/:token/renew` (token is the credential; `Guardianship#resend_invite!`, 10-minute `RENEW_COOLDOWN`); accepted/revoked keep the generic message | Parent had no recovery path | `app/controllers/guardianships_controller.rb`, `config/routes.rb`, new `app/views/guardianships/expired.html.erb` |
| Wrong-account: renders a page (403) with the redacted invited address and "Sign out and open the link again" (`users#logout`) | Flash + bounce to a teen dashboard | `app/controllers/guardianships_controller.rb`, `app/views/guardianships/show.html.erb` |
| `/billing`: an invited-but-unsigned guardian is told to accept first (that is what sets `known_adult?`) and linked to `/guardian` | Was told "ask your parent" | `app/controllers/fuime/billing_controller.rb`, `app/views/fuime/billing/show.html.erb`, `spec/requests/fuime_billing_spec.rb` |
...
## 2026-09-12 — The family setup wizard (both entry orders)

Onboarding had one entry order — a teen starts, and a parent is invited by an
email triggered from deep inside an application form — and it ran on HCB's
nonprofit intake wizard with Fuime copy on top. A parent who wanted to set the
family up had nowhere to go: `GuardianshipPolicy#new?` refuses known adults,
`GuardianshipsController#create` hardcodes `minor: current_user`, and
`site/parents.html`'s only CTA was the waitlist (ONBOARDING_PLAN §2, §5 E1).

`/setup` is both orders over one controller. Five screens for a teen, three for
a parent, converging on the same venture. The rule that shapes every screen:
**under merchant-of-record the guardian gates PAYOUTS** — not signing up, not
setting up, not selling — so the teen's parent step sends the invite and moves
on, and the only thing that waits on a parent is money leaving. The gate that
IS before selling is Fuime's own human vetting, and every screen says so
rather than implying it away.

No ledger, money path, agreement text or Stripe call changed. One additive
migration.

| Change | Why | Files |
|---|---|---|
| `Fuime::OnboardingController` — `start` dispatcher + teen (`you` `business` `name` `family` `done`) + parent (`teen` `sign` `done`); session holds a draft id and an unvalidated name/email, never a consent | One controller, both orders. Nothing legally significant lives only in the session: every attestation and consent is written to its record in the request it is given | new `app/controllers/fuime/onboarding_controller.rb`, `app/views/fuime/onboarding/**`, `config/routes.rb` |
| Persists through the SAME interfaces: `User#attest_minor_13_plus`, `Event::Application` + `mark_submitted!`, `Fuime::FounderAdmission`, `Fuime::GuardianInviteService` (via the application's own after-hook), `Guardianship#accept!` | The wizard is a new front end over the shipped flow, not a second implementation of it. `Event::ApplicationsController` is untouched and still serves adults and second ventures | — |
| **`Fuime::GuardianConsentService`** — the 18+ attestation, `activation_blockers` and `accept!` in the one order that works; `GuardianshipsController#accept` and the wizard's `sign` both call it | ONBOARDING_PLAN §3 pins `adult_18_plus` to ONE path. A second screen with the same checkbox would have made two. The order is load-bearing: attest before reading blockers (it is the blocker being cleared) and through the association (`attest_adult_18_plus!` uses `update_columns`, so a sibling instance holds a stale value) | new `app/services/fuime/guardian_consent_service.rb`, `app/controllers/guardianships_controller.rb` |
| **The parent door refuses any account that ticked 13+ during a founder signup** (`GuardianshipPolicy#create_as_guardian?` and the GET guard) | Without it a teenager could finish their own signup, walk to `/setup/parent`, invent an email, tick the agreement, and come out `known_adult?` — self-serve adulthood in two clicks, and the escalation `Guardianship.signed_up_as_a_young_founder?` closes at the accept page. Stricter than that predicate on purpose: it also requires a venture or guardianship, because an INVITED parent who ticked 13+ must still be able to accept. Nobody invited this one. A genuine parent-first parent never sees that checkbox | `app/policies/guardianship_policy.rb`, `app/controllers/fuime/onboarding_controller.rb` |
| `guardianships.initiated_by` (`minor: 0`, `guardian: 1`); `accept!(notify_minor:)`; `self_signed_signals` skips fast-acceptance for a guardian-initiated row | A parent-first row is created and accepted in one request, so (a) "your parent accepted your invitation" is nonsense to a teen who never sent one, and (b) `accepted_suspiciously_fast?` would fire on every ordinary family and bury the real signals. The other four signals still apply to them | new `db/migrate/20260912140000_add_initiated_by_to_guardianships.rb`, `app/models/guardianship.rb` |
| `Fuime::FamilyMailer#teen_join` + `Fuime::FamilyInviteService` (signed id, purpose `:family_join`, 7 days) + `Fuime::FamilyInvitesController` (`GET /join/:token`) + `SignedLinkSignIn` | The parent-first teen's one email. The token signs its holder in AS THE MINOR, so it is minted inside the mailer and appears in no view, flash, session or log. Sign-in goes through `Login` + `ProcessLoginService` (2FA preserved) — the waitlist admit's mechanism, now shared rather than duplicated | new mailer/service/controller/concern, `app/controllers/waitlist_invites_controller.rb` |
| `Fuime::MinorMailWindow` — `teen_join` is scheduled for 06:00 local when it would otherwise send in the quiet hours | L7: transactional only, nothing to a minor 12–6 a.m. The app runs in UTC, so a naive hour check is somebody else's midnight. Pre-existing minor mail (`Event::ApplicationReminderJob`, `GuardianshipMailer#accepted`) is unchanged — a known gap, not solved here | new `app/lib/fuime/minor_mail_window.rb` |
| One front door: `redirect_to_onboarding`, `LoginsController#complete` and `WaitlistInvitesController` all send a name-less user to `/setup`; `GuardianshipsController` skips the redirect on `show`/`accept` and the accept page collects a missing name | The settings form asked a person who had never seen Fuime to fill in a profile. The skip is NOT optional and must ship in the same commit: without it a name-less parent on `/guardian/:token` bounces between the wizard and the agreement forever | `app/controllers/application_controller.rb`, `logins_controller.rb`, `waitlist_invites_controller.rb`, `guardianships_controller.rb`, `app/views/guardianships/show.html.erb` |
| **L8 correction: the guardian approves each payout request.** Said on all three parent screens, the teen's family screen and the join mail | `PayoutRequest#approver_must_be_the_responsible_adult` and `EventPolicy#decide_payout?` both require an overseeing guardian. Phase A removed "you approve every payout" because the weekly `Fuime::PayoutBatch` is admin-approved — true, but it left the copy understating the parent's actual control, which is both false and the most persuasive thing the parent path has to say | `app/views/fuime/onboarding/**`, `app/views/fuime/family_mailer/*` |
| A revoked pair is NOT refused at `sign` | Migration `20260912120000` scoped the uniqueness index to live rows precisely so a parent who withdrew consent can be asked again without support. The parent signing again IS the recovery | `app/controllers/fuime/onboarding_controller.rb` |
| Every rejection of the teen-email field returns one sentence; `POST /setup/parent/*` throttled to 10/hour per session | A distinct "that account is 18 or older" would confirm both that an account exists at a guessed address and that it is an adult's. `save_sign` calls `User.create!`, so an unthrottled session could seed unlimited stubs | `app/controllers/fuime/onboarding_controller.rb`, `config/initializers/rack_attack.rb` |
| The wizard is a FIRST venture only — a founder with a venture and a draft goes to the old status page | Its `family` screen says nothing about the free plan's one-venture limit, so a founder would meet `activation_blockers` only at submit | `app/controllers/fuime/onboarding_controller.rb` |
| New layout `fuime_setup` + `_wizard.scss` + `wizard_controller.js` (Stimulus) | `no_app_shell` never renders `application/_footer`, so the standing "not a bank" disclosure is rendered by the layout from a **new `application/_status_disclosure`** partial extracted byte-for-byte — eight pages where somebody signs a legal agreement about money, and 12 CFR 328.102(b)(3)(ii) makes the omission the violation. Motion is entirely optional: every screen is a server-rendered form, nothing calls `preventDefault`, `prefers-reduced-motion` is honoured, and no element rests at `opacity: 0`. `.btn--fuime` and every fill read `map-get($palette, primary)` ($fuime-blue) — Tailwind's `primary` is HCB red | new `app/views/layouts/fuime_setup.html.erb`, `app/views/fuime/onboarding/_rail.html.erb`, `app/assets/stylesheets/components/_wizard.scss`, `app/javascript/controllers/wizard_controller.js`, `app/views/application/_status_disclosure.html.erb`, `_footer.html.erb` |
| `guardianships/_agreement_box` shared by the accept page and the wizard's `sign` | Two screens can now produce a signature; the checkbox id, the label wording and the scroll box must not drift, because the consent record does not say which screen produced it. Agreement partials and `CURRENT_AGREEMENT_VERSION` (`2026-09-11-v4`) untouched | `app/views/guardianships/{show,_agreement_box}.html.erb` |
| Playground: **Priya** (`playground+priya@fuime.test`) + her stub kid, `Fuime::Playground.persona?`, `reset_new_parent!`, `Admin::PlaygroundController#fresh_parent`, and a mail interceptor that drops any message addressed only to personas | The playground runs on the production deploy and this wizard sends real mail. Two layers: the wizard refuses a persona typing a non-`@fuime.test` address on screen, the interceptor is the backstop for every path that does not ask | `app/services/fuime/playground.rb`, `app/lib/fuime/playground_mail_interceptor.rb`, `config/initializers/fuime_playground_mail.rb`, `app/controllers/admin/playground_controller.rb`, `app/views/admin/playground/show.html.erb` |

**Not done here:** the `?k=` signed sign-in token on the guardian invite (C1's
